	...
	if ($login) {
		if (!$_POST['text'] || $_POST['text'] == " ") {
			...
			if (!$text || $text == " ") {
				...
				print "<p class=\"er\">You can not add a comment more than once every 5 minutes.</p>";
			} else {
				...
				if (mysql_query($sql)) {
					print "<p class=\"erok\">Comment added!</p>";
...
print '<h2>Comments to revocation:</h2>';
...
function topics_list($page, $num, $status)
{
	...
	if(!$themes) {
		print '<p class="er">To this review No comments.</p>';
	}
...
print '	<div style="margin-top: 3px;" class="hline"></div>
		<div align="right">
			<a style="color: red;" href="?a=pollno&pid='.$row['id'].'">I do not agree ['.$row['poll_no'].']</a> | <a style="color: green;" href="?a=pollyes&pid='.$row['id'].'">Agree ['.$row['poll_yes'].']</a>
		</div>
			</td>
		</tr>
...
<table width="380" border="0" align="center">
	...
	<tr>
		<td><font color="red"><b>!</b></font> The comment text:<br /><textarea style="width: 378px;" name="text" rows="7" cols="48"><?php print htmlspecialchars(substr($_POST['text'], 0, 10000), ENT_QUOTES, ''); ?></textarea></td>
	</tr>
	<tr>
...
<?php
			...
			print '<p class="er">To leave a comment you need to login!</p>';
		}

	} else {
		print "<p class=\"er\">Review not found!</p>";
	}
?>